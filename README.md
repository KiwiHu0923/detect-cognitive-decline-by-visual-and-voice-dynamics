...
- **Overview** — [What ParkScreen Does](#what-parkscreen-does) · [Why this matters](#why-this-matters)
- **Evidence** — [Headline Result](#headline-result)
- **Run it** — [Quick Start](#quick-start)
- **Method** — [Architecture](#architecture) · [Repository Layout](#repository-layout)
- **Trust & caveats** — [Data and Attribution](#data-and-attribution) · [Limitations](#limitations) · [Reproducibility](#reproducibility)
...
Parkinson's is a motor-speech disorder: sustained-vowel and DDK acoustics have been shown to discriminate PD from healthy controls with clinically usable accuracy [Little et al. 2009], and the smile-task facial protocol we use is drawn directly from prior PD-screening work [Islam et al. 2023]. That's why jitter/shimmer/HNR (phonation) and DDK rate/regularity (articulation) are **direct** evidence — not proxies for a general cognitive or motor decline. Late-fusing two task-matched channels beats either alone because they measure the same disease through orthogonal motor systems (vocal-fold vibration vs. tongue–lip coordination). The table below is the headline of that claim.
...
- **Layer 2 (real-time, on user upload):** the fitted Layer-1 classifiers plus a separately-trained facial classifier (on UFNet) score each channel; AUC-excess weighting fuses them; structured context passes to Claude Opus 4.7 for the Markdown report.
